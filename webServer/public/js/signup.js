        ...
        $.ajax({
            ...
            beforeSend: function() {
                if (checkEmailFmt(formData.email) !== "ok") {
                    ...
                    return false
                } else if (checkNicknameFmt(formData.nickname) !== "ok") {
                    errMsg(checkNicknameFmt(formData.nickname))
                    return false
                } else if (checkPwdFmt(formData.pwd1) !== "ok") {
                    errMsg(checkPwdFmt(formData.pwd1))
                    return false
                } else if (checkPwdConfirm(formData.pwd1, formData.pwd2) !== "ok") {
                    errMsg(checkPwdConfirm(formData.pwd1, formData.pwd2))
                    return false
                }
            },
...
//                     return false
//                 } else if (checkNicknameFmt(formData.nickname) !== "ok") {
//                     console.log(checkNicknameFmt(formData.nickname))
